...
public class DirtyFlagMap<K, V> implements Map<K, V>, Cloneable, java.io.Serializable {
    ...
    public boolean equals(final Object obj) {
        if (!(obj instanceof DirtyFlagMap)) {
            return false;
        }
    ...
    private class DirtyFlagMapEntrySet extends DirtyFlagSet<Entry<K, V>> {
        ...
        public <U> U[] toArray(final U[] array) {
            ...
            U[] result = array.length < size ? (U[]) Array.newInstance(array.getClass().getComponentType(), size) : array;

            Iterator<Entry<K, V>> entryIter = iterator(); // Will return
            // DirtyFlagMapEntry
            // objects
