 ...
 * @Source 1.0
 */
public class TriggerActionContainer {

    private static final Map<String, Class<?>> map = new HashMap<>();


    public static void register() {
        Set<Class<?>> triggerActionClazzs = ClassScanningUtil.findClassBySuper(TriggerActionContainer.class.getPackageName(), AbstractTriggerAction.class);
    ...
    public static <T extends TriggerAction<?>> T create(String className, TriggerActionWrap actionWrap) throws RuntimeException {
        Class<?> clazz = map.get(className);
        Objects.requireNonNull(clazz, className + "nonexistent");
        try {
            return (T) clazz.getDeclaredConstructor(TriggerActionWrap.class).newInstance(actionWrap);
        } catch (Exception e) {
